...
function BoardsListPage() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      setBoards([]); // Clear boards list on new search
    }, 300);

    return () => clearTimeout(timer);
  }, [search]);

  // Reset page and boards list when filters or sorting change
  useEffect(() => {
    setPage(1);
    setBoards([]);
  }, [sort, showFavorites]);
  ...
  const boardsQuery = rqClient.useQuery('get', '/boards', {
    params: {
      query: {
        ...
        isFavorite: showFavorites === null ? undefined : showFavorites,
      },
    },
    enabled: true,
  });

  // Update boards list when receiving new data
  useEffect(() => {
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="mb-8">
        <form
          ...
          <div className="flex-grow">
            ...
            <Input id="board-name" name="name" placeholder="Enter name..." />
          </div>
      ...
      {boardsQuery.isPending && page === 1 ? (
        ...
        <>
            ...
            {boards.map((board) => (
              <Card key={board.id} className="relative">
                <div className="absolute top-2 right-2 flex items-center gap-2">
                  ...
                  <span className="text-sm text-gray-500">{board.isFavorite ? 'Favorited' : ''}</span>
                </div>
                <CardHeader>
          ...
          {boards.length === 0 && !boardsQuery.isPending && <div className="text-center py-10">No boards found</div>}
