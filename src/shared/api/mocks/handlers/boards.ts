...
// Function to generate random board name
...
function generateRandomBoards(count: number): ApiSchemas['Board'][] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const updatedAt = new Date(new Date(createdAt).getTime() + Math.random() * 86400000 * 10).toISOString(); // Add up to 10 days
    const lastOpenedAt = new Date(new Date(updatedAt).getTime() + Math.random() * 86400000 * 5).toISOString(); // Add up to 5 days
    ...
    result.push({
      ...
      updatedAt,
      lastOpenedAt,
      isFavorite: Math.random() > 0.7, // Approximately 30% of boards will be favorites
    });
  }
...
export const boardsHandlers = [
    ...
    if (sort) {
      filteredBoards.sort((a, b) => {
        if (sort === 'name') {
          ...
          // For dates (createdAt, updatedAt, lastOpenedAt)
          return (
  ...
  http.get('/boards/{boardId}', async ({ params, request }) => {
    ...
    // Update lastOpenedAt when viewing board
